...
class StaffToScore(object):
    def __init__(self, args):
        ...
        # Initialize model and load checkpoint with device mapping
        self.model = TrOMR(args)
        checkpoint = torch.load(args.filepaths.checkpoint, map_location=self.device)
        # Load state dict with strict=False to bypass key mismatches
        self.model.load_state_dict(checkpoint, strict=False)
        self.model.to(self.device)
        
        # Initialize tokenizers and image transformation
    ...
    # Define preprocessing as an instance method to access self attributes
